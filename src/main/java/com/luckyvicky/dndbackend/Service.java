...
public class Service {
    ...
    public Service(OdsayProperties odSayProperties, OdsayOpenFeign odsayOpenFeign) {
        ...
        randomGotchaMap.put(10_000, List.of(new RandomGotcha(3, "카페 음료 사기"), new RandomGotcha(4, "영화 값 내주기")));
        randomGotchaMap.put(30_000, List.of(new RandomGotcha(5, "점심 값 내주기"), new RandomGotcha(6, "미술관 티켓")));
        randomGotchaMap.put(50_000, List.of(new RandomGotcha(7, "저녁 값 내주기"), new RandomGotcha(8, "소품샵 굿즈")));
        randomGotchaMap.put(100_000, List.of(new RandomGotcha(9, "대전에서 만나기"), new RandomGotcha(10, "하루 재워주기")));
    ...
    public Destinations getDestination(String userId) {
        log.info(userId);
        ...
        log.info("response: {}", odsaySearchResponse);

        return Destinations.of(
            mapPosition.startName(),
            mapPosition.endName(),
            odsaySearchResponse.result().path());
    }
    ...
    public Results getResult(String userId) {
        ...
        Results results = Results.of(selectedDestination.vehicles(), endName, totalMinutes, totalPayment, jokeByPrice, jokeByTime, randomGotchas,
    ...
    public void saveFinalResult(FinalResultRequest finalResultRequest, String userId) {
        ...
        List<RandomGotcha> randomGotchas = results.data().getRandomGotchas();
    ...
    public void saveGotcha(Gotcha gotcha, String userId) {
        if (gotcha.customGotcha() == null) {
            return;
        }
        var results = resultsMap.get(userId);
        List<RandomGotcha> randomGotchas = results.data().getRandomGotchas();
        var newGotchas = new ArrayList<RandomGotcha>(randomGotchas);
        newGotchas.add(new RandomGotcha(Integer.MAX_VALUE, gotcha.customGotcha()));
        results.setGotcha(newGotchas);
    }
}
